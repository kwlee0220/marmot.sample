...
public class WeakFireDeathArea {
	...
	public static final void main(String... args) throws Exception {
		...
		Plan plan0 = marmot.planBuilder("서울시 종합병원 위치에서 3km 버퍼 연산")
								...
								.build();
		marmot.createDataSet("tmp/종합병원_위치_추천/병원_3km버퍼",  plan0, StoreDataSetOptions.create().geometryColumnInfo(gcInfo).force(true));
		...
		Plan plan1 = marmot.planBuilder("종합병원 3km 밖의 서울시 영역 계산")
								...
								.build();
		marmot.createDataSet("tmp/종합병원_위치_추천/병원_원거리_영역", plan1, StoreDataSetOptions.create().geometryColumnInfo(gcInfo).force(true));
		...
		plan = marmot.planBuilder("seoul_fire_death")
					...
					.build();
		result = marmot.createDataSet("tmp/종합병원_위치_추천/서울_화재", plan, StoreDataSetOptions.create().geometryColumnInfo(gcInfo).force(true));
